...
class DastyarC_Dashboard {
	...
	/** آیا ابزارک فعال است؟ — v1.9.1: سوییچ‌های نمایش از فرم تنظیمات حذف شد ← همه بخش‌ها همیشه فعال */
	...
	public function render_page() {
		...
		printf(
			...
			esc_url( admin_url( 'admin.php?page=dastyarc-hub&tab=stats' ) )
		);
	...
	public static function setup_steps() {
		...
		return array(
			array(
				...
				'done'   => $configured,
				'url'    => admin_url( 'admin.php?page=dastyarc-hub&tab=settings' ),
			...
			array(
				...
				'done'   => $has_product,
				'url'    => admin_url( 'admin.php?page=dastyarc-hub&tab=products' ),
	...
	public function render_connection() {
		...
		if ( ! $configured ) {
			...
			echo '<p><a class="button button-primary dcd-btn" href="' . esc_url( admin_url( 'admin.php?page=dastyarc-hub&tab=settings' ) ) . '">رفتن به تنظیمات اتصال</a></p>';
			return;
		}
		...
		echo '<a class="button" href="' . esc_url( admin_url( 'admin.php?page=dastyarc-hub&tab=products' ) ) . '">محصولات دستیار</a>';
		echo '<a class="button" href="' . esc_url( admin_url( 'admin.php?page=dastyarc-hub&tab=settings' ) ) . '">تنظیمات</a>';
	...
	public function render_rma() {
		...
		echo '<div class="dcd-grid">';
		$this->stat_card( (string) (int) $new, 'گزارش جدید (ارسال‌نشده)', admin_url( 'admin.php?page=dastyarc-hub&tab=rma' ), 'amber' );
		$this->stat_card( (string) (int) $listening, 'در حال بررسی مرکز', admin_url( 'admin.php?page=dastyarc-hub&tab=rma' ), 'green' );
		$this->stat_card( (string) (int) $waiting, 'در جریان عودت کالا', admin_url( 'admin.php?page=dastyarc-hub&tab=rma' ), 'green' );
		echo '</div>';
		...
		if ( $recent ) {
			...
			foreach ( $recent as $p ) {
				...
				printf(
					...
					esc_url( admin_url( 'admin.php?page=dastyarc-hub&tab=rma&view=' . (int) $p->ID ) ),
					(int) $p->ID,
		...
		echo '<p class="dcd-links"><a class="button button-primary dcd-btn" href="' . esc_url( admin_url( 'admin.php?page=dastyarc-hub&tab=rma' ) ) . '">مدیریت گزارش‌های عودت</a></p>';
	}
