...
class DastyarC_Rma {
	...
	public function handle_send() {
		...
		if ( is_wp_error( $res ) ) {
			wp_safe_redirect( add_query_arg( array( 'page' => 'dastyarc-rma', 'view' => $rma_id, 'rma_error' => rawurlencode( $res->get_error_message() ) ), admin_url( 'admin.php' ) ) );
		} else {
			wp_safe_redirect( admin_url( 'admin.php?page=dastyarc-rma&view=' . $rma_id . '&rma_sent=1' ) );
		}
		exit;
	}
	...
	public function handle_refresh() {
		...
		if ( is_wp_error( $res ) ) {
			...
			wp_safe_redirect( admin_url( 'admin.php?page=dastyarc-rma&view=' . $rma_id . '&rma_synced=1' ) );
		}
		exit;
	}
	...
	public function handle_note() {
		...
		wp_safe_redirect( admin_url( 'admin.php?page=dastyarc-rma&view=' . $rma_id . '&rma_note_saved=1' ) );
		exit;
	}
	...
	public function handle_manual() {
		...
		if ( is_wp_error( $res ) ) {
			wp_safe_redirect( add_query_arg( array( 'page' => 'dastyarc-rma', 'new' => 1, 'order_id' => $order_id, 'rma_error' => rawurlencode( $res->get_error_message() ) ), admin_url( 'admin.php' ) ) );
		} else {
			wp_safe_redirect( admin_url( 'admin.php?page=dastyarc-rma&view=' . (int) $res . '&rma_created=1' ) );
		}
		exit;
	}
	...
	protected function page_admin_list() {
		echo '<div class="wrap"><h1 class="wp-heading-inline">عودت و مرجوعی</h1> ';
		printf( '<a href="%s" class="page-title-action">+ ثبت گزارش عودت (دستی)</a>', esc_url( admin_url( 'admin.php?page=dastyarc-rma&new=1' ) ) );
		...
		echo '<input type="hidden" name="page" value="dastyarc-rma">';
		...
		foreach ( $posts as $p ) {
			...
			printf( '<td><a class="button button-small" href="%s">مدیریت</a></td>', esc_url( admin_url( 'admin.php?page=dastyarc-rma&view=' . (int) $p->ID ) ) );
			echo '</tr>';
		}
	...
	protected function page_admin_new() {
		echo '<div class="wrap"><h1>ثبت گزارش عودت (دستی)</h1>';
		printf( '<p><a href="%s">← بازگشت به لیست</a></p>', esc_url( admin_url( 'admin.php?page=dastyarc-rma' ) ) );
		$this->notices_admin();
		...
		if ( ! $order_id ) {
			...
			echo '<input type="hidden" name="page" value="dastyarc-rma"><input type="hidden" name="new" value="1">';
	...
	protected function page_admin_detail( $rma_id ) {
		...
		echo '<div class="wrap">';
		printf( '<h1>عودت #%d <a class="page-title-action" href="%s">← بازگشت به لیست</a></h1>', (int) $rma_id, esc_url( admin_url( 'admin.php?page=dastyarc-rma' ) ) );
		$this->notices_admin();
