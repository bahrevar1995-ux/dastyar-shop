...
class DastyarC_Rma {
	...
	protected function notices_admin() {
		if ( isset( $_GET['rma_sent'] ) ) {
			echo '<div class="notice notice-success is-dismissible"><p>درخواست عودت برای دستیار شاپ ارسال شد.</p></div>';
		}
		...
		if ( isset( $_GET['rma_created'] ) ) {
			echo '<div class="notice notice-success is-dismissible"><p>گزارش عودت ثبت شد.</p></div>';
		}
	...
	protected function page_admin_new() {
		...
		if ( ! $order->get_meta( '_dastyarc_remote_order_id' ) ) {
			echo '<div class="notice notice-warning"><p>این سفارش هنوز به دستیار شاپ ارسال نشده است.</p></div></div>';
			return;
		}
		...
		echo '<p><label><strong>توضیحات فروشنده (اختیاری)</strong></label><br><textarea name="vendor_note" rows="3" style="width:100%"></textarea></p>';
	...
	protected function page_admin_detail( $rma_id ) {
		...
		if ( ! $remote ) {
			...
			echo '<p><label><strong>توضیحات فروشنده</strong></label><br>';
		...
		if ( ! $remote ) {
			echo '<p class="description">هنوز ارسال نشده است.</p>';
		} else {
	...
	protected function handle_customer_submit() {
		...
		return $this->public_msg( sprintf( '✔ گزارش عودت شما با شناسه #%d ثبت شد.', (int) $rma_id ), false );
	}
