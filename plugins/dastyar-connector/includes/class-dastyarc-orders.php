...
class DastyarC_Orders {
	...
	const MIXED_STATUS     = 'dastyar-mixed';     // (v1.9.2) «سفارش ترکیبی» — بخشی از اقلام دستیار و بخشی فروشنده
	const SUSPENDED_STATUS = 'dastyar-suspended'; // (v1.9.2) «تعلیق شده» توسط مرکز — اتصال قطع و مدیریت سفارش واگذار به فروشنده
	...
	public function register_status() {
		...
		// (v1.9.2) «سفارش ترکیبی» — قسمتی از اقلام به مرکز رفته و قسمتی با فروشنده مانده است
		register_post_status( 'wc-' . self::MIXED_STATUS, array(
			'label'                     => 'سفارش ترکیبی',
			'public'                    => false,
			'exclude_from_search'       => false,
			'show_in_admin_all_list'    => true,
			'show_in_admin_status_list' => true,
			'label_count'               => _n_noop( 'سفارش ترکیبی <span class="count">(%s)</span>', 'سفارش ترکیبی <span class="count">(%s)</span>' ),
		) );
		// (v1.9.2) «تعلیق شده» — مرکز سفارش را به تعلیق برده؛ اتصال با مرکز قطع و مدیریت کامل سفارش با فروشنده است
		register_post_status( 'wc-' . self::SUSPENDED_STATUS, array(
			'label'                     => 'تعلیق شده',
			'public'                    => false,
			'exclude_from_search'       => false,
			'show_in_admin_all_list'    => true,
			'show_in_admin_status_list' => true,
			'label_count'               => _n_noop( 'تعلیق شده <span class="count">(%s)</span>', 'تعلیق شده <span class="count">(%s)</span>' ),
		) );
	}
	...
	public function add_status( $statuses ) {
		...
		foreach ( $statuses as $slug => $label ) {
			...
			if ( 'wc-processing' === $slug ) {
				...
				$new[ 'wc-' . self::MIXED_STATUS ] = 'سفارش ترکیبی';
			}
		}
		foreach ( array( self::SENT_STATUS => 'ارسال شده به دستیارشاپ', self::MIXED_STATUS => 'سفارش ترکیبی', self::SUSPENDED_STATUS => 'تعلیق شده' ) as $s => $l ) {
			if ( ! isset( $new[ 'wc-' . $s ] ) ) {
				$new[ 'wc-' . $s ] = $l;
			}
		}
	...
	/** این وضعیت‌ها به معنای «پرداخت‌شده»‌اند تا رفتار ووکامرس (ایمیل/گزارش/قابلیت‌ها) حفظ شود */
	public function paid_statuses( $statuses ) {
		$statuses[] = self::SENT_STATUS;
		$statuses[] = self::MIXED_STATUS;
		return $statuses;
	}
	...
	/** آیا این قلم متعلق به دستیار (مرکز) است؟ — همان قاعده نگاشت محصول/متغیر به مرکز + رعایت «خروج از دراپ‌شیپینگ» */
	public static function item_is_remote( $item ) {
		...
		if ( $var_id && get_post_meta( $var_id, '_dastyar_remote_var_id', true ) ) {
			...
			return ! ( $parent && get_post_meta( $parent, '_dastyarc_detached', true ) );
		}
		return (bool) ( $pid && get_post_meta( $pid, '_dastyar_remote_id', true ) && ! get_post_meta( $pid, '_dastyarc_detached', true ) );
	}

	/** تفکیک اقلام سفارش به «دستیار» و «فروشنده» — پایه منطق «سفارش ترکیبی» (v1.9.2) */
	public static function order_parts( WC_Order $order ) {
		$remote = 0;
		$local  = 0;
		foreach ( $order->get_items() as $item ) {
			if ( self::item_is_remote( $item ) ) {
		...
		return array( 'remote' => $remote, 'local' => $local );
	}

	/** آیا مرکز سفارش را «تعلیق» کرده و اتصالش قطع شده است؟ (v1.9.2) */
	public static function is_link_broken( $order ) {
		return $order instanceof WC_Order && (bool) $order->get_meta( '_dastyarc_link_broken' );
	}

	/** برچسب فارسی وضعیت هر «بخش» از سفارش ترکیبی (v1.9.2) */
	public static function part_label( $part ) {
		...
		return $map[ $part ] ?? (string) $part;
	}

	/**
	 * دروازه تکمیل سفارش ترکیبی (v1.9.2): وقتی «بخش دستیار» و «بخش فروشنده» هر دو به «تکمیل شده» رسیدند،
	 * سفارش به‌صورت خودکار wc-completed می‌شود؛ تا آن لحظه در حالت «سفارش ترکیبی» با وضعیت منفصل هر بخش می‌ماند.
	 */
	public static function maybe_complete_mixed( $order ) {
		if ( ! $order instanceof WC_Order ) {
			return;
		}
		if ( ! $order->get_meta( '_dastyarc_mixed' ) || self::is_link_broken( $order ) ) {
			return;
		}
		if ( 'completed' !== $order->get_status()
			&& 'completed' === (string) $order->get_meta( '_dastyarc_part_center' )
			&& 'completed' === (string) $order->get_meta( '_dastyarc_part_vendor' ) ) {
			$order->update_status( 'completed', 'سفارش ترکیبی: هر دو بخش (دستیار و فروشنده) به اتمام رسید.' );
		}
	}
	...
	public function push_cancel( $order_id, $from = '' ) {
		...
		if ( self::is_link_broken( $order ) ) {
			return; // (v1.9.2) سفارش «تعلیق‌شده» است؛ اتصال با مرکز قطع شده و دیگر هیچ همگام‌سازی‌ای انجام نمی‌شود
		}
	...
	public static function status_label( $slug ) {
		...
		$map  = array(
			...
			self::MIXED_STATUS     => 'سفارش ترکیبی',
			self::SUSPENDED_STATUS => 'تعلیق شده',
		);
		if ( isset( $map[ $slug ] ) ) {
			return $map[ $slug ];
		}
	...
	public static function send( WC_Order $order ) {
		...
		if ( $parts['local'] > 0 ) {
			// (v1.9.2) «سفارش ترکیبی»: فقط اقلام دستیار به مرکز رفتند؛ بخش فروشنده همچنان با خود فروشنده است.
			// از اینجا به‌بعد وضعیت/کد رهگیری هر بخش جداگانه مدیریت می‌شود تا هر دو «تکمیل» شوند.
			$order->update_meta_data( '_dastyarc_mixed', 1 );
			$order->update_meta_data( '_dastyarc_part_center', 'processing' );
			$order->update_meta_data( '_dastyarc_part_vendor', 'pending' );
			$order->add_order_note( sprintf(
				'سفارش ترکیبی: %d قلم دستیار به مرکز ارسال شد (سفارش مرکزی #%s) و %d قلم این فروشگاه همچنان با شماست. تا تکمیل هر دو بخش، سفارش در وضعیت «سفارش ترکیبی» باقی می‌ماند.',
				(int) $parts['remote'],
				$result['number'],
				(int) $parts['local']
			) );
		...
		// تغییر وضعیت محلی به «ارسال شده به دستیارشاپ» یا «سفارش ترکیبی»
		if ( in_array( $order->get_status(), array( 'processing', 'on-hold' ), true ) ) {
			$order->set_status( $parts['local'] > 0 ? self::MIXED_STATUS : self::SENT_STATUS );
		}
