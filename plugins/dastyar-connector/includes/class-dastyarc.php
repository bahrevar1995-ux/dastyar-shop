...
final class DastyarC {
	...
	public static function log( $message, $level = 'info' ) {
		...
		// (v1.10.0) حلقه رویدادهای سبک برای کارت «رویدادهای اخیر» داشبورد فرماندهی — ۱۵۰ مورد آخر (v1.12.0)
		...
		// (v1.12.0) ایمیل خطا (opt-in از تنظیمات — حداکثر هر ۶ ساعت یک‌بار)
		if ( 'error' === $level && 'yes' === get_option( 'dastyarc_notify_email', 'no' ) && ! get_transient( 'dastyarc_errmail' ) ) {
			set_transient( 'dastyarc_errmail', 1, 6 * HOUR_IN_SECONDS );
			wp_mail( (string) get_option( 'admin_email' ), 'خطای کانکتور دستیار شاپ', (string) $message );
		}
	}
