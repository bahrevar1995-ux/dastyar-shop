...
class DastyarC_Sync {
	...
	public function run() {
		...
		for ( $page = 1; $page <= $max_page; $page++ ) {
			...
			// (v1.12.0) یک کوئری برای نگاشت همه آیتم‌های صفحه به‌جای یک کوئری برای هر آیتم
			$rids = array();
			...
			$lmap = DastyarC_Importer::local_map( $rids );
			foreach ( $items as $item ) {
				// (v1.11.0) تحمل آیتم بدشکل در پاسخ مرکز — هیچ‌وقت نباید به خطای مهم برسد
				if ( ! is_array( $item ) ) {
					continue;
				}
				if ( empty( $lmap[ (int) ( $item['id'] ?? 0 ) ] ) ) {
					continue;
				}
