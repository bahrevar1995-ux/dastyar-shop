 ...
 * اعتبارسنجی: نسخه ۲ (HMAC روی ts.body + پنجره زمانی + ضدتکرار) با سازگاری نسخه ۱ (v1.12.0)
 */
if ( ! defined( 'ABSPATH' ) ) {
	exit;
}
...
class DastyarC_Listener {
	...
	public function handle( WP_REST_Request $request ) {
		...
		$sig2 = (string) $request->get_header( 'x-dastyar-signature-v2' );
		$ts   = (string) $request->get_header( 'x-dastyar-timestamp' );
		$key  = (string) get_option( 'dastyarc_api_key' );

		$verified = false;
		$v1       = false;
		if ( '' !== $key && '' !== $sig2 && '' !== $ts ) {
			// (v1.12.0) امضای نسخه ۲: تازگی (۵ دقیقه) + ضدتکرار
			if ( abs( time() - (int) $ts ) <= 300 && hash_equals( hash_hmac( 'sha256', $ts . '.' . $body, $key ), $sig2 ) ) {
				$jti = 'dastyarc_wh_' . sha1( $ts . '.' . $body );
				if ( ! get_transient( $jti ) ) {
					set_transient( $jti, 1, 600 );
					$verified = true;
				} else {
					DastyarC::log( 'Webhook rejected: replay detected', 'warning' );
					return new WP_Error( 'dastyarc_replay', 'درخواست تکراری است.', array( 'status' => 401 ) );
				}
			}
		}
		if ( ! $verified && 'yes' === get_option( 'dastyarc_webhook_v1_fallback', 'yes' ) && '' !== $key && '' !== $sig ) {
			// سازگاری با مرکز قدیمی
			if ( hash_equals( hash_hmac( 'sha256', $body, $key ), $sig ) ) {
				$verified = true;
				$v1       = true;
			}
		}
		...
		if ( $v1 && ! get_transient( 'dastyarc_wh_v1warn' ) ) {
			set_transient( 'dastyarc_wh_v1warn', 1, DAY_IN_SECONDS );
			DastyarC::log( 'Webhook v1 received — ask center to upgrade for replay protection', 'warning' );
		}
