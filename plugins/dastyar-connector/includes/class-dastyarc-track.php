...
class DastyarC_Track {
 ...
 protected function find_by_phone( $phone ) {
	...
	protected function result( $order, $error ) {
		...
		// (v1.9.2) «سفارش ترکیبی»: کد رهگیریِ بخش فروشنده به‌طور جدا و متمایز از کد مرکز نمایش داده می‌شود
		$is_mixed  = (bool) $order->get_meta( '_dastyarc_mixed' );
		$v_track   = (string) $order->get_meta( '_dastyarc_vendor_tracking_code' );
		$v_carrier = (string) $order->get_meta( '_dastyarc_vendor_tracking_carrier' );
		...
		<div class="dastyarc-track-result">
			<table>
				...
				<?php if ( $tracking ) : ?>
				<tr>
					<td><?php echo $is_mixed ? 'کد رهگیری مرسوله (بخش دستیار)' : 'کد رهگیری مرسوله (دستیار)'; ?></td>
					<td>
				...
				<?php endif; ?>
				<?php if ( $is_mixed && $v_track ) : /* (v1.9.2) بخش فروشنده سفارش ترکیبی */ ?>
				<tr>
					<td>کد رهگیری مرسوله (بخش فروشگاه)</td>
					<td>
						<span class="dastyarc-track-code"><?php echo esc_html( $v_track ); ?></span>
						<?php echo $v_carrier ? '<br><small>روش ارسال: ' . esc_html( $v_carrier ) . '</small>' : ''; ?>
					</td>
				</tr>
				<?php endif; ?>
				<?php if ( $self_track && ( ! $tracking || $order->get_meta( '_dastyarc_link_broken' ) ) ) : /* (v1.9.2) تعلیق‌شده: کد فروشنده همیشه دیده شود */ ?>
				<tr>
					<td>کد رهگیری مرسوله<?php echo $order->get_meta( '_dastyarc_link_broken' ) ? ' (فروشگاه)' : ''; ?></td>
					<td>
