...
class DastyarC_Importer {
	...
	/** @var string آخرین مرحله ایمپورت (لاگ تشخیصی v1.11.0) */
	protected static $last_step = '';

	/** @var array کش درون‌ریکوئستی نگاشت شناسه مرکزی → محصول محلی (رفع N+1 در سینک — v1.12.0) */
	protected static $local_map_cache = array();

	/** شناسه محصول محلی متناظر با محصول مرکزی (0 = وجود ندارد) */
	public static function local_id( $remote_id ) {
		$remote_id = (int) $remote_id;
		if ( array_key_exists( $remote_id, self::$local_map_cache ) ) {
			return self::$local_map_cache[ $remote_id ];
		}
		$ids = get_posts( array(
			'post_type'      => 'product',
			'post_status'    => 'any',
		...
		$found = $ids ? (int) $ids[0] : 0;
		self::$local_map_cache[ $remote_id ] = $found;
		return $found;
	}

	/**
	 * نگاشت دسته‌جمعی شناسه‌های مرکزی → محصول محلی با یک کوئری (v1.12.0)
	 * @return array [remote_id => local_id]
	 */
	public static function local_map( array $remote_ids ) {
		$remote_ids = array_values( array_filter( array_unique( array_map( 'intval', $remote_ids ) ) ) );
		if ( ! $remote_ids ) {
			return array();
		}
		$found = get_posts( array(
			...
			'meta_query'     => array( // phpcs:ignore WordPress.DB.SlowDBQuery.slow_db_query_meta_query
				array( 'key' => '_dastyar_remote_id', 'value' => $remote_ids, 'compare' => 'IN' ),
			),
		) );
		$map = array();
		foreach ( (array) $found as $pid ) {
			$rid = (int) get_post_meta( $pid, '_dastyar_remote_id', true );
			if ( $rid ) {
				$map[ $rid ] = (int) $pid;
				self::$local_map_cache[ $rid ] = (int) $pid;
			}
		}
		foreach ( $remote_ids as $rid ) {
			if ( ! array_key_exists( $rid, self::$local_map_cache ) ) {
				self::$local_map_cache[ $rid ] = 0;
			}
		}
		return $map;
	}
	...
	public static function import_remote( $remote_id, $new_status = 'draft' ) {
		// (v1.11.0) همه مسیرهای ایمپورت (تکی/دسته‌جمعی/وب‌هوک/کرون): سقف زمانی و حافظه بالاتر
		...
		// (v1.11.1) همه مسیرهای ایمپورت (تکی/دسته‌جمعی/وب‌هوک/کرون): خرابی مرحله دریافت هم خطای تمیز می‌دهد
		try {
			$data = DastyarC_Client::product( $remote_id );
		} catch ( \Throwable $e ) {
			DastyarC::log( sprintf( 'Import fetch crashed (remote #%d): %s', $remote_id, $e->getMessage() ), 'error' );
			return new WP_Error( 'dastyarc_fetch_failed', 'دریافت اطلاعات محصول از مرکز ناموفق بود؛ اتصال و لاگ خطا بررسی شود.' );
		}
		if ( is_wp_error( $data ) ) {
			return $data;
		}
	...
	public static function save( array $data, $new_status = 'draft' ) {
		// (v1.11.0) هیچ خطایی از ایمپورت نباید به «خطای مهم» وردپرس برسد: هر استثنای
		// غیرمنتظره (TypeError در PHP8، هوک افزونه ثالث روی ذخیره، …) این‌جا گرفته و به
		// WP_Error تمیز + لاگ تشخیصی (مرحله/نسخه‌ها/حافظه) تبدیل می‌شود.
		self::$last_step = 'start';
		try {
			return self::save_unsafe( $data, $new_status );
		} catch ( \Throwable $e ) {
			DastyarC::log( sprintf(
				'Import crashed (remote #%d, step=%s): %s in %s:%d | PHP %s / WC %s / mem %s',
				(int) ( $data['id'] ?? 0 ),
				self::$last_step,
				$e->getMessage(),
			...
			return new WP_Error( 'dastyarc_import_failed', 'افزودن محصول ناموفق بود؛ اتصال و لاگ خطا بررسی شود.' );
		}
	}

	/**
	 * منطق اصلی ذخیره — فقط از save() صدا زده می‌شود (پوشش try/catch + ردیابی مرحله)
	 * @return int|WP_Error شناسه محصول محلی
	 */
	protected static function save_unsafe( array $data, $new_status = 'draft' ) {
		...
		// (v1.12.0) قفل محتوا: نام/توضیحات دستی فروشنده بازنویسی نمی‌شود
		if ( $is_new || ! get_post_meta( (int) $local_id, '_dastyarc_lock_content', true ) ) {
		...
		// (v1.11.1) اگر ایمپورت قبلی در مرحله تصاویر سوخته باشد (پرچم images_ok نیست)، دوباره تلاش می‌شود
		if ( ! $unchanged || $is_new || 'yes' !== get_post_meta( $product_id, '_dastyarc_images_ok', true ) ) {
			self::$last_step = 'images';
			// (v1.11.1) خرابی مرحله تصاویر (مثلاً cURL ناقص هاست یا هوک افزونه ثالث روی آپلود)
			// نباید کل ایمپورت را بسوزاند: محصول ذخیره می‌شود، تصاویر رد می‌شوند و در ایمپورت
			// بعدی خودکار دوباره تلاش می‌شود.
			try {
				self::set_images( $product_id, (array) ( $data['images'] ?? array() ) );
				update_post_meta( $product_id, '_dastyarc_images_ok', 'yes' );
			} catch ( \Throwable $e ) {
				delete_post_meta( $product_id, '_dastyarc_images_ok' );
				$hint = function_exists( 'curl_multi_init' ) ? '' : ' — cURL این هاست ناقص است؛ از هاست بخواهید cURL را کامل فعال کند';
				DastyarC::log( sprintf( 'Import #%d: images skipped (%s)%s', $product_id, $e->getMessage(), $hint ), 'warning' );
			}
		}

		if ( 'variable' === $type ) {
			self::$last_step = 'variations';
	...
	protected static function apply_price_and_stock( $product, $supplier_price, array $remote, $is_new = true ) {
		...
		// (v1.12.0) قفل قیمت/موجودی محصول (برای تنوع: قفل والد هم لحاظ می‌شود)
		$lock_pid   = (int) $product->get_id();
		$lock_ppid  = ( method_exists( $product, 'get_parent_id' ) ) ? (int) $product->get_parent_id() : 0;
		$lock_price = (bool) get_post_meta( $lock_pid, '_dastyarc_lock_price', true ) || ( $lock_ppid && get_post_meta( $lock_ppid, '_dastyarc_lock_price', true ) );
		$lock_stock = (bool) get_post_meta( $lock_pid, '_dastyarc_lock_stock', true ) || ( $lock_ppid && get_post_meta( $lock_ppid, '_dastyarc_lock_stock', true ) );

		if ( ( ! $manual_mode || $is_new ) && ! $lock_price ) {
		...
		if ( 'yes' === get_option( 'dastyarc_stock_sync', 'yes' ) && ! $lock_stock ) {
	...
	public static function apply_stock_only( $local_id, array $remote ) {
		...
		// (v1.12.0) قفل موجودی محصول
		if ( get_post_meta( (int) $local_id, '_dastyarc_lock_stock', true ) ) {
			return false;
		}
	...
	public static function apply_price_now( $local_id ) {
		...
		// (v1.12.0) قفل قیمت محصول (شامل اعمال دستی صف قیمت)
		if ( get_post_meta( $local_id, '_dastyarc_lock_price', true ) ) {
			return false;
		}
	...
	protected static function set_terms( $product, array $terms, $taxonomy ) {
		$ids = array();
		foreach ( $terms as $t ) {
			// (v1.11.0) تحمل شکل غیراستاندارد: رشته ساده هم به‌عنوان نام پذیرفته می‌شود
			$name = sanitize_text_field( (string) ( is_array( $t ) ? ( $t['name'] ?? '' ) : $t ) );
			if ( '' === $name ) {
				continue;
			}
	...
	protected static function save_variations( $product_id, array $data, $is_new = true ) {
		...
		$vars_total = is_array( $data['variations'] ?? null ) ? count( (array) $data['variations'] ) : 0;
		$vars_saved = 0;
		foreach ( (array) ( $data['variations'] ?? array() ) as $v ) {
			...
			foreach ( (array) ( $v['attributes'] ?? array() ) as $at ) {
				// (v1.11.0) اعتبارسنجی شکل ویژگی + اصلاح مقدار WP_Error از get_term
				if ( ! is_array( $at ) ) {
					continue;
				}
				$at_tax   = (string) ( $at['taxonomy'] ?? '' );
				$at_name  = (string) ( $at['name'] ?? '' );
				$at_value = (string) ( $at['value'] ?? '' );
				if ( '' === $at_value || '' === ( $at_tax . $at_name ) ) {
					continue;
				}
				if ( '' !== $at_tax && taxonomy_exists( $at_tax ) ) {
					$term = get_term_by( 'name', $at_value, $at_tax );
					if ( ! $term ) {
						$created = wp_insert_term( $at_value, $at_tax );
						$term    = is_wp_error( $created ) ? null : get_term( (int) ( $created['term_id'] ?? 0 ) );
					}
					if ( $term && ! is_wp_error( $term ) && ! empty( $term->slug ) ) {
						$attrs[ $at_tax ] = $term->slug;
					}
				} else {
					$attrs[ $at_name ] = $at_value;
				}
			}
			...
			$lock_pv = (bool) get_post_meta( (int) $product_id, '_dastyarc_lock_price', true );
			$lock_sv = (bool) get_post_meta( (int) $product_id, '_dastyarc_lock_stock', true );

			if ( ( ! $manual_mode || $is_new || ! $local_v->get_id() ) && ! $lock_pv ) {
			...
			if ( 'yes' === get_option( 'dastyarc_stock_sync', 'yes' ) && ! $lock_sv ) {
		...
		if ( $vars_total > 0 && ! $vars_saved ) {
			DastyarC::log( sprintf( 'Import #%d: product saved but none of %d variations could be saved.', $product_id, $vars_total ), 'warning' );
		}
	...
	protected static function set_images( $product_id, array $images ) {
		...
		foreach ( $images as $img ) {
			// (v1.11.0) تحمل شکل غیراستاندارد: رشته ساده هم به‌عنوان نشانی تصویر پذیرفته می‌شود
			$src = esc_url_raw( (string) ( is_array( $img ) ? ( $img['src'] ?? '' ) : $img ) );
			if ( ! $src ) {
				continue;
			}
			$aid = self::sideload( $src, $product_id, is_array( $img ) ? (string) ( $img['alt'] ?? '' ) : '' );
			if ( $aid ) {
				$attachment_ids[] = $aid;
			}
		}
	...
	protected static function sideload( $url, $product_id, $alt = '' ) {
		// (v1.11.0) ساخت بندانگشتی تصویر حافظه‌بر است؛ سقف را بالا ببر تا در هاست ضعیف «خطای مهم» ندهد
		if ( function_exists( 'wp_raise_memory_limit' ) ) {
