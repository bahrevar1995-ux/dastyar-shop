...
final class DastyarC_Ui {
	...
	public static function css() {
		...
		.dcx2-card.dcx2-hero{display:block;background:var(--grad);border:0;box-shadow:0 14px 34px -14px rgba(23,161,109,.5);padding:18px;border-radius:18px}
		...
		.dcx2-hero .tiles{display:grid;grid-template-columns:repeat(auto-fit,minmax(220px,1fr));gap:12px;margin-top:14px}
		...
		@media(max-width:760px){.dcx2-hero .tiles{grid-template-columns:1fr}}
		/* ───────── منوی ⋮ ستون اقدام (v1.11.0) — پاپ‌اپ شناور مشترک ───────── */
		.dcx2-dots{width:32px;height:32px;border-radius:50%;border:1.5px solid var(--line);background:#fff;color:var(--mut);cursor:pointer;display:inline-flex;align-items:center;justify-content:center;padding:0}
		.dcx2-dots:hover{border-color:var(--g1);color:var(--g1);background:#f2faf6}
		#dcx2-pop{position:fixed;z-index:999999;min-width:190px;background:#fff;border:1px solid var(--line);border-radius:12px;box-shadow:0 14px 34px -10px rgba(31,34,51,.25);padding:6px;display:none}
		#dcx2-pop.open{display:block}
		#dcx2-pop a{display:flex;align-items:center;gap:8px;padding:8px 12px;border-radius:8px;font-size:12px;font-weight:700;color:var(--ink);white-space:nowrap}
		#dcx2-pop a:hover{background:#f2faf6;color:var(--g1)}
		#dcx2-pop a.danger{color:#c81e38}
		#dcx2-pop a.danger:hover{background:#fdf0f0}
		</style>
		<?php
	}
