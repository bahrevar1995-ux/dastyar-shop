...
class Dastyar_Install {
	...
	public static function tables() {
		...
		CREATE TABLE {$wpdb->prefix}dastyar_webhook_log (
			id bigint(20) unsigned NOT NULL AUTO_INCREMENT,
			vendor_id bigint(20) unsigned NOT NULL DEFAULT 0,
			event varchar(60) NOT NULL DEFAULT '',
			payload text NULL,
			code varchar(190) NOT NULL DEFAULT '',
	...
	public static function install() {
		...
		if ( ! wp_next_scheduled( 'dastyar_webhook_retry' ) ) {
			wp_schedule_event( time() + 300, 'dastyar_five_minutes', 'dastyar_webhook_retry' );
		}
		flush_rewrite_rules();
	...
	/**
	 * ارتقای نصب‌های موجود (v1.12.0): جدول‌ها + زمان‌بندی‌های جاافتاده — یک مقایسه ارزان در هر ریکوئست
	 */
	public static function maybe_upgrade() {
		if ( (string) get_option( 'dastyar_core_version', '' ) === (string) DASTYAR_CORE_VERSION ) {
			return;
		}
		self::tables();
		if ( ! wp_next_scheduled( 'dastyar_webhook_retry' ) ) {
	...
	public static function deactivate() {
		...
		wp_clear_scheduled_hook( 'dastyar_webhook_retry' );
		flush_rewrite_rules();
	}
}
