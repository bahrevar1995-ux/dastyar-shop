...
final class Dastyar_Statuses {
	...
	// (v1.11.1 — درخواست کاربر) «تعلیق شده»: سفارش از چرخه دستیار خارج می‌شود — نه موفق و نه لغوشده؛
	// مدیریت کامل سفارش به فروشنده (مالک محصول) واگذار می‌گردد و سفارش از اتصال با دستیار جدا می‌شود.
	const SUSPENDED     = 'dastyar-suspended';      // ← wc-dastyar-suspended
	const SUSPENDED_LBL = 'تعلیق شده';
	...
	/** کلید کامل وضعیت تعلیق‌شده با پیشوند wc- (v1.11.1) */
	public static function slug_suspended() {
		return 'wc-' . self::SUSPENDED;
	}
	...
	public static function register() {
		...
		// (v1.11.1) «تعلیق شده» — خروج سفارش از چرخه دستیار بدون لغو؛ مدیریت بر عهده فروشنده
		register_post_status( self::slug_suspended(), array(
			'label'                     => self::SUSPENDED_LBL,
			'public'                    => false,
			'exclude_from_search'       => false,
			'show_in_admin_all_list'    => true,
			'show_in_admin_status_list' => true,
			/* translators: %s: شمارش سفارش‌ها */
			'label_count'               => _n_noop( self::SUSPENDED_LBL . ' <span class="count">(%s)</span>', self::SUSPENDED_LBL . ' <span class="count">(%s)</span>', 'dastyar-core' ),
		) );
	}

	/** درج در دراپ‌داون وضعیت‌ها، بلافاصله بعد از «در حال انجام» — «تعلیق شده» ابتدای بخش پایانی وضعیت‌ها */
	public static function add( $statuses ) {
		...
		// (v1.11.1) «تعلیق شده» انتهای فهرست (نزدیک وضعیت‌های پایانی چون «لغو شده») قرار می‌گیرد
		if ( ! isset( $new[ self::slug_suspended() ] ) ) {
			$new[ self::slug_suspended() ] = self::SUSPENDED_LBL;
		}
		return $new;
	}
	...
	public static function bulk_action( $actions ) {
		...
		$ours = array(
			'mark_' . self::POSTED    => sprintf( 'تغییر وضعیت به «%s»', self::LABEL ),
			'mark_' . self::SUSPENDED => sprintf( 'تغییر وضعیت به «%s»', self::SUSPENDED_LBL ), // (v1.11.1)
		);
		$new  = array();
		$done = false;
		foreach ( (array) $actions as $k => $v ) {
			if ( 'trash' === $k && ! $done ) {
				$new = array_merge( $new, $ours ); // قبل از «حذف»
				$done = true;
			}
			$new[ $k ] = $v;
		}
	...
	/** رنگ زرد «تحویل پست شده» + قرص قرمز یاسی «تعلیق شده» (v1.11.1) در لیست سفارش‌های پیشخوان */
	public function list_css() {
		...
		echo '<style>.order-status.status-posted,.column-order_status .status-posted{background:#ffe9a8 !important;color:#6b4e00 !important;border-bottom-color:#f0d060 !important}'
			. '.order-status.status-dastyar-suspended,.column-order_status .status-dastyar-suspended{background:#f9d7db !important;color:#8f1b2c !important;border-bottom-color:#eeadb5 !important}</style>';
	}
}
