 ...
 * نسخه ۲ (v1.12.0): X-Dastyar-Signature-V2 = HMAC-SHA256(ts.body) + هدر X-Dastyar-Timestamp
 */
if ( ! defined( 'ABSPATH' ) ) {
	exit;
}
...
class Dastyar_Webhooks {
	 ...
	 */
	public function send( $vendor_id, $event, array $data, $blocking = true, $order = null, $is_retry = false ) {
		...
		$response = wp_remote_post( $url, array(
			...
			'headers'  => array(
				...
				'X-Dastyar-Signature-V2' => hash_hmac( 'sha256', $ts . '.' . $body, $key ),
				'X-Dastyar-Timestamp'    => $ts,
		...
		if ( $error ) {
			...
			if ( ! $is_retry ) {
				self::queue_failed( $vendor_id, $event, $data, (string) $error );
			}
	...
	/* ------------------------------------------------------------------
	 * لاگ تحویل + تلاش مجدد (v1.12.0) — جدول dastyar_webhook_log
	 * ---------------------------------------------------------------- */

	/** ثبت ارسال ناموفق (مسدودکننده) در صف تلاش مجدد */
	public static function queue_failed( $vendor_id, $event, array $data, $error ) {
		global $wpdb;
		$now = current_time( 'mysql' );
		$wpdb->insert(
			$wpdb->prefix . 'dastyar_webhook_log',
			array(
				'vendor_id'  => (int) $vendor_id,
				'event'      => substr( (string) $event, 0, 60 ),
				'payload'    => wp_json_encode( $data ),
				'code'       => substr( (string) $error, 0, 190 ),
				'attempts'   => 1,
				'status'     => 'queued',
				'next_try'   => gmdate( 'Y-m-d H:i:s', time() + self::backoff( 1 ) ),
				'created_at' => $now,
				'updated_at' => $now,
			),
	...
	/** فاصله تلاش بعدی برحسب ثانیه: ۵ دقیقه، ۳۰ دقیقه، ۲ ساعت، ۱۲ ساعت */
	public static function backoff( $attempts ) {
		$steps = array( 300, 1800, 7200, 43200 );
		return $steps[ min( max( 0, (int) $attempts - 1 ), 3 ) ];
	}

	/** پردازش صف (کرون ۵ دقیقه‌ای) — حداکثر ۲۰ مورد در هر اجرا */
	public function process_retries() {
		global $wpdb;
		$table = $wpdb->prefix . 'dastyar_webhook_log';
		$rows  = $wpdb->get_results( $wpdb->prepare(
			"SELECT * FROM {$table} WHERE status = %s AND next_try <= %s ORDER BY next_try ASC LIMIT 20", // phpcs:ignore
			'queued', current_time( 'mysql' )
		) );
		foreach ( (array) $rows as $row ) {
			$res = $this->send( (int) $row->vendor_id, (string) $row->event, (array) json_decode( (string) $row->payload, true ), true, null, true );
			if ( is_wp_error( $res ) ) {
				$att = (int) $row->attempts + 1;
				if ( $att >= 5 ) {
					$wpdb->update( $table,
						array( 'attempts' => $att, 'status' => 'failed', 'code' => substr( $res->get_error_message(), 0, 190 ), 'updated_at' => current_time( 'mysql' ) ),
						array( 'id' => (int) $row->id ), array( '%d', '%s', '%s', '%s' ), array( '%d' ) );
				} else {
					$wpdb->update( $table,
						array( 'attempts' => $att, 'next_try' => gmdate( 'Y-m-d H:i:s', time() + self::backoff( $att ) ), 'code' => substr( $res->get_error_message(), 0, 190 ), 'updated_at' => current_time( 'mysql' ) ),
						array( 'id' => (int) $row->id ), array( '%d', '%s', '%s', '%s' ), array( '%d' ) );
				}
			} else {
				$wpdb->update( $table,
					array( 'status' => 'sent', 'code' => '', 'updated_at' => current_time( 'mysql' ) ),
					array( 'id' => (int) $row->id ), array( '%s', '%s', '%s' ), array( '%d' ) );
			}
		}
		// هرس خودکار: ارسال‌شده/ناموفق‌های قدیمی‌تر از ۳۰ روز
		$wpdb->query( $wpdb->prepare(
			"DELETE FROM {$table} WHERE status IN ('sent','failed') AND updated_at < %s", // phpcs:ignore
			gmdate( 'Y-m-d H:i:s', time() - 30 * DAY_IN_SECONDS )
		) );
	}

	/** تلاش مجدد دستی از پیشخوان */
	public function retry_now( $id ) {
		global $wpdb;
		$table = $wpdb->prefix . 'dastyar_webhook_log';
		$row   = $wpdb->get_row( $wpdb->prepare( "SELECT * FROM {$table} WHERE id = %d", (int) $id ) ); // phpcs:ignore
		if ( ! $row ) {
			return false;
		}
		$res = $this->send( (int) $row->vendor_id, (string) $row->event, (array) json_decode( (string) $row->payload, true ), true, null, true );
		if ( is_wp_error( $res ) ) {
			$wpdb->update( $table,
				array( 'attempts' => (int) $row->attempts + 1, 'status' => 'queued', 'next_try' => gmdate( 'Y-m-d H:i:s', time() + self::backoff( (int) $row->attempts + 1 ) ), 'code' => substr( $res->get_error_message(), 0, 190 ), 'updated_at' => current_time( 'mysql' ) ),
				array( 'id' => (int) $row->id ), array( '%d', '%s', '%s', '%s', '%s' ), array( '%d' ) );
			return false;
		}
		$wpdb->update( $table,
			array( 'status' => 'sent', 'code' => '', 'updated_at' => current_time( 'mysql' ) ),
	...
	/** خواندن لاگ برای پیشخوان */
	public static function get_log( $limit = 100, $status = '' ) {
		global $wpdb;
		$table = $wpdb->prefix . 'dastyar_webhook_log';
		$limit = max( 1, min( 500, (int) $limit ) );
		if ( in_array( $status, array( 'queued', 'sent', 'failed' ), true ) ) {
			return $wpdb->get_results( $wpdb->prepare( "SELECT * FROM {$table} WHERE status = %s ORDER BY id DESC LIMIT {$limit}", $status ) ); // phpcs:ignore
		}
		return $wpdb->get_results( "SELECT * FROM {$table} ORDER BY id DESC LIMIT {$limit}" ); // phpcs:ignore
	}
}
