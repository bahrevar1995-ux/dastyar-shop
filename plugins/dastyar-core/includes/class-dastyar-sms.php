...
class Dastyar_Sms {
	...
	public static function defaults() {
		return array(
			...
			'pattern_settled'      => '',     // v1.12.0 — تسویه ماهانه به فروشنده
	...
	public function __construct() {
		...
		add_action( 'dastyar_settled', array( $this, 'on_settled' ), 20, 2 ); // v1.12.0 — تسویه ماهانه به فروشنده
	}
	...
	/** تسویه ماهانه فروشنده (v1.12.0) */
	public function on_settled( $uid, $month = '' ) {
		$this->notify_user( (int) $uid, 'settled', array(
			'name'  => self::display_name( (int) $uid ),
			'month' => (string) $month,
		) );
	}
