...
class Dastyar_Admin {
	...
	public function __construct() {
		...
		add_action( 'admin_post_dastyar_webhook_retry', array( $this, 'webhook_retry' ) );
		...
		add_action( 'admin_post_dastyar_save_ctr_update', array( $this, 'save_ctr_update' ) ); // v1.11.2 — مانیفست به‌روزرسانی کانکتور
	...
	public function menu() {
		...
		add_submenu_page( 'dastyar', 'لاگ وب‌هوک‌ها', 'وب‌هوک‌ها', 'manage_woocommerce', 'dastyar-webhooks', array( $this, 'page_webhooks' ) );
	...
	/** لاگ تحویل وب‌هوک‌ها + تلاش مجدد دستی (v1.12.0) */
	public function page_webhooks() {
		echo '<div class="wrap"><h1>لاگ وب‌هوک‌ها</h1>';
		if ( ! empty( $_GET['wh_retried'] ) ) {
			echo '<div class="notice notice-success is-dismissible"><p>تلاش مجدد انجام شد.</p></div>';
		}
		$f      = sanitize_key( $_GET['wstatus'] ?? '' );
		$base   = admin_url( 'admin.php?page=dastyar-webhooks' );
		$labels = array( '' => 'همه', 'queued' => 'در صف', 'sent' => 'ارسال‌شده', 'failed' => 'ناموفق' );
		echo '<p>';
		foreach ( $labels as $k => $t ) {
			printf( '<a class="button%s" style="margin-left:6px" href="%s">%s</a>', $f === $k ? ' button-primary' : '', esc_url( '' === $k ? $base : add_query_arg( 'wstatus', $k, $base ) ), esc_html( $t ) );
		}
		echo '</p>';
		$rows = Dastyar::instance()->webhooks->get_log( 100, $f );
		$pill = array( 'queued' => '#d98a1f', 'sent' => '#17a16d', 'failed' => '#c81e38' );
		$txt  = array( 'queued' => 'در صف', 'sent' => 'ارسال شد', 'failed' => 'ناموفق' );
		echo '<table class="widefat striped"><thead><tr><th>زمان</th><th>فروشنده</th><th>رویداد</th><th>وضعیت</th><th>تلاش</th><th>تلاش بعدی</th><th>خطا</th><th></th></tr></thead><tbody>';
		foreach ( (array) $rows as $r ) {
			$user = $r->vendor_id ? get_userdata( (int) $r->vendor_id ) : null;
			$st   = isset( $pill[ (string) $r->status ] ) ? (string) $r->status : 'queued';
			printf(
				'<tr><td>%s</td><td>%s</td><td style="direction:ltr;text-align:right">%s</td><td><span style="display:inline-block;background:%s;color:#fff;border-radius:99px;padding:1px 12px;font-size:11px">%s</span></td><td>%d</td><td>%s</td><td>%s</td><td>%s</td></tr>',
				esc_html( Dastyar_Jalali::dt( (string) $r->created_at ) ),
				$user ? esc_html( $user->display_name ) : '—',
				esc_html( (string) $r->event ),
				esc_attr( $pill[ $st ] ),
				esc_html( $txt[ $st ] ),
				(int) $r->attempts,
				'queued' === $st ? esc_html( Dastyar_Jalali::dt( (string) $r->next_try ) ) : '—',
				esc_html( (string) $r->code ),
				'sent' === $st ? '' : sprintf( '<a class="button button-small" href="%s">تلاش مجدد</a>', esc_url( wp_nonce_url( admin_url( 'admin-post.php?action=dastyar_webhook_retry&id=' . (int) $r->id ), 'dastyar_webhook_retry' ) ) )
			);
		}
		echo '</tbody></table></div>';
	}

	/** تلاش مجدد دستی یک وب‌هوک ناموفق */
	public function webhook_retry() {
		if ( ! current_user_can( 'manage_woocommerce' ) || ! wp_verify_nonce( sanitize_key( $_GET['_wpnonce'] ?? '' ), 'dastyar_webhook_retry' ) ) {
			wp_die( 'نشست نامعتبر' );
		}
		$id = (int) ( $_GET['id'] ?? 0 );
		if ( $id ) {
			Dastyar::instance()->webhooks->retry_now( $id );
		}
		wp_safe_redirect( admin_url( 'admin.php?page=dastyar-webhooks&wh_retried=1' ) );
		exit;
	}

	/** وضعیت سامانه + تنظیمات فرستنده (برای لیبل آدرس) */
	public function page_status() {
		if ( ! empty( $_GET['ctr_saved'] ) ) {
			echo '<div class="notice notice-success is-dismissible" style="max-width:900px"><p>✔ تنظیمات به‌روزرسانی کانکتور ذخیره شد و برای همه فروشگاه‌های متصل ارسال گردید.</p></div>';
		}
		...
		/* ── (v1.11.2 — درخواست کاربر) اطلاع‌رسانی به‌روزرسانی افزونه کانکتور به پیشخوان فروشگاه‌ها ── */
		$m      = Dastyar_Rest::update_manifest();
		$labels = Dastyar_Rest::update_labels();
		echo '<hr><h2>🔄 اطلاع‌رسانی به‌روزرسانی کانکتور فروشنده</h2>';
		echo '<p class="description" style="max-width:900px">با ذخیره این فرم، فروشگاه‌های متصل در «نوار ابزار بالای پیشخوان وردپرس» خودشان اعلان به‌روزرسانی را با برچسب اولویتِ انتخابی شما می‌بینند و می‌توانند افزونه «دستیارشاپ کانکتور» را با <strong>یک کلیک</strong> از همان پیشخوان خود به‌روز کنند. (به‌محض ذخیره، اعلان فوری هم برای همه فروشندگان متصل Push می‌شود.)</p>';
		if ( '' !== $m['version'] ) {
			echo '<div class="notice notice-info inline" style="max-width:900px;padding:8px 14px">';
			printf( '<p style="margin:0">نسخه فعلیِ منتشرشده: <strong dir="ltr">%s</strong> — برچسب: <strong>%s</strong>%s</p>',
				esc_html( $m['version'] ),
				esc_html( $m['label_title'] ),
				$m['released'] ? ' — تاریخ انتشار: ' . esc_html( $m['released'] ) : ''
			);
			echo '</div>';
		} else {
			echo '<div class="notice inline" style="max-width:900px;padding:8px 14px"><p style="margin:0">هنوز هیچ به‌روزرسانی‌ای تعریف نشده است.</p></div>';
		}
		echo '<form method="post" action="' . esc_url( admin_url( 'admin-post.php' ) ) . '">';
		wp_nonce_field( 'dastyar_save_ctr_update' );
		echo '<input type="hidden" name="action" value="dastyar_save_ctr_update">';
		echo '<table class="form-table">';
		printf(
			'<tr><th><label>نسخه جدید کانکتور</label></th><td><input type="text" name="ctr_version" class="regular-text" value="%s" style="direction:ltr" placeholder="1.9.3"><p class="description">اگر این نسخه از نسخه نصب‌شده در سایت فروشنده بیشتر باشد، اعلان نمایش داده می‌شود. برای حذف کامل اعلان، خالی بگذارید.</p></td></tr>',
			esc_attr( $m['version'] )
		);
		printf(
			'<tr><th><label>لینک فایل ZIP نسخه جدید</label></th><td><input type="url" name="ctr_package" class="large-text" value="%s" style="direction:ltr;text-align:left" placeholder="https://…/dastyar-connector-1.9.3.zip"><p class="description">فایل ZIP را در «رسانه‌ها» همین سایت (یا هر هاست دلخواه) آپلود کنید و لینک مستقیمش را این‌جا بگذارید — نصب با کلیک فروشنده از همین لینک دانلود می‌شود.</p></td></tr>',
			esc_attr( $m['package'] )
		);
		echo '<tr><th><label>برچسب اولویت</label></th><td><select name="ctr_label">';
		foreach ( $labels as $key => $title ) {
			$hint = array(
				'normal'    => 'اعلان فقط در نوار ابزار + فهرست افزونه‌ها آبی/خاکستری',
				'important' => 'اعلان نارنجی + هشدار در صفحه افزونه‌ها',
				'critical'  => 'اعلان قرمز + هشدار ماندگار در تمام صفحات پیشخوان',
				'urgent'    => 'اعلان قرمز چشمک‌زن + هشدار ماندگار در تمام صفحات',
			)[ $key ];
			printf( '<option value="%s" %s>%s</option>', esc_attr( $key ), selected( $m['label'], $key, false ), esc_html( $title . ' — ' . $hint ) );
		}
		echo '</select></td></tr>';
		printf(
			'<tr><th><label>متن اعلان</label></th><td><textarea name="ctr_message" rows="2" class="large-text">%s</textarea><p class="description">یک‌خط، در تول‌تیپ نوار ابزار و داخل هشدار پیشخوان فروشنده نمایش داده می‌شود. مثلاً: «رفع باگ سینک قیمت — حتماً به‌روز کنید».</p></td></tr>',
			esc_textarea( $m['message'] )
		);
		printf(
			'<tr><th><label>شرح تغییرات (هر سطر یک مورد)</label></th><td><textarea name="ctr_changelog" rows="5" class="large-text" placeholder="هر سطر یک تغییر؛ به‌صورت لیست در مودال تغییرات نمایش داده می‌شود">%s</textarea></td></tr>',
			esc_textarea( $m['changelog'] )
		);
		echo '</table>';
		submit_button( 'ذخیره و ارسال اعلان برای همه فروشگاه‌ها' );
		echo '</form>';
	...
	public function page_sms() {
		...
		$sms = Dastyar_Sms::all();
		echo '<div class="wrap"><h1>📱 پیامک <small style="font-weight:400;color:#8a97a3;font-size:12px">(اطلاع‌رسانی پیامکی تأیید عضویت، نتیجه مرجوعی، کد رهگیری به مشتری، اتمام شارژ کیف پول، تسویه ماهانه و کد ورود)</small></h1>';
		...
		$events = array(
			...
			'settled'    => array( 'تسویه ماهانه فروشنده', 'متغیرهای معمول: name, month' ),
		);
	...
	/** (v1.11.2) ذخیره مانیفست به‌روزرسانی کانکتور + انتشار فوری برای همه فروشگاه‌های متصل */
	public function save_ctr_update() {
		if ( ! current_user_can( 'manage_woocommerce' ) || ! wp_verify_nonce( sanitize_key( $_POST['_wpnonce'] ?? '' ), 'dastyar_save_ctr_update' ) ) {
			wp_die( 'دسترسی غیرمجاز' );
		}
		$version = sanitize_text_field( wp_unslash( $_POST['ctr_version'] ?? '' ) );
		$package = esc_url_raw( wp_unslash( $_POST['ctr_package'] ?? '' ) );
		$label   = sanitize_text_field( wp_unslash( $_POST['ctr_label'] ?? 'normal' ) );
		if ( ! isset( Dastyar_Rest::update_labels()[ $label ] ) ) {
			$label = 'normal';
		}
		$manifest = array(
			...
			'package'   => $package,
			'label'     => $label,
			'message'   => sanitize_textarea_field( wp_unslash( $_POST['ctr_message'] ?? '' ) ),
			'changelog' => sanitize_textarea_field( wp_unslash( $_POST['ctr_changelog'] ?? '' ) ),
			'released'  => current_time( 'Y/m/d - H:i' ),
		);
		update_option( 'dastyar_ctr_update', $manifest, false );

		// انتشار فوری (غیرمسدودکننده) مانیفست جدید برای همه فروشندگان؛ سایت فروشنده هم‌زمان کش دوره‌ای هم دارد
		Dastyar::instance()->webhooks->push_connector_update();

		wp_safe_redirect( admin_url( 'admin.php?page=dastyar-status&ctr_saved=1' ) );
		exit;
	}
	...
	public function save_sms() {
		...
		foreach ( array( 'approved', 'rejected', 'rma', 'wallet_low', 'otp', 'tracking', 'settled' ) as $ev ) {
