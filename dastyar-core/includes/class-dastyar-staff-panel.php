...
class Dastyar_Staff_Panel {
		...
		<div class="dop-shell">
			<aside class="dop-side">
				<div class="dop-side-head">
					<div class="dop-logo">دستیار<span>شاپ</span></div>
					<button type="button" class="dop-mobtoggle" onclick="document.querySelector('.dop-nav').classList.toggle('open')" aria-label="باز کردن منو"><?php echo self::icon( 'menu' ); ?></button>
				</div>
				<p class="dop-tag">پنل عملیات</p>
				<nav class="dop-nav">
			...
			<main class="dop-main">
				<div class="dop-topbar">
					<button type="button" class="dop-darktoggle" onclick="dopToggleDark()" title="حالت تاریک/روشن"><?php echo self::icon( 'moon' ); ?></button>
					<div class="dop-topbar-user">
						<span class="dop-user"><?php echo esc_html( wp_get_current_user()->display_name ); ?></span>
	...
	protected static function sec_dashboard() {
		...
		$orders     = wc_get_orders( array( 'limit' => 300, 'date_created' => '>=' . $today_from ) );
		$sum        = 0;
		...
		echo '<h1 class="dop-title">داشبورد</h1>';
		echo '<div class="dop-kpis">';
		echo self::kpi( 'box', 'سفارش امروز', Dastyar_Panel_Settings::fa_num( count( (array) $orders ) ), 'blue' );
		echo self::kpi( 'wallet', 'فروش امروز', wp_kses_post( wc_price( $sum ) ), 'green' );
	...
	/** کارت برجسته رشد فروش این هفته نسبت به هفته قبل */
	protected static function weekly_growth_card() {
		...
		$sum_this = 0;
		foreach ( (array) wc_get_orders( array( 'limit' => 300, 'date_created' => '>=' . $this_from, 'status' => array( 'wc-processing', 'wc-completed', 'wc-posted', 'wc-on-hold' ) ) ) as $o ) {
			$sum_this += (float) $o->get_total();
		}
		$sum_last = 0;
		$range    = gmdate( 'Y-m-d H:i:s', $last_from ) . '...' . gmdate( 'Y-m-d H:i:s', $last_to );
		foreach ( (array) wc_get_orders( array( 'limit' => 300, 'date_created' => $range, 'status' => array( 'wc-processing', 'wc-completed', 'wc-posted', 'wc-on-hold' ) ) ) as $o ) {
		...
		printf(
			'<div class="dop-growth%s"><div class="dop-growth-ic">%s</div><div><p>رشد فروش این هفته نسبت به هفته قبل</p><b>%s%s٪</b></div></div>',
			$down ? ' down' : '',
			self::icon( $down ? 'trenddown' : 'trend' ),
			$down ? '' : '+',
			Dastyar_Panel_Settings::fa_num( abs( $pct ) )
		);
	}
	...
	protected static function kpi( $icon, $label, $value, $kind = 'green' ) {
		return '<div class="dop-kpi kpi-' . esc_attr( $kind ) . '"><div class="dop-kpi-ic">' . self::icon( $icon ) . '</div><b>' . $value . '</b><span>' . esc_html( $label ) . '</span></div>';
	}
